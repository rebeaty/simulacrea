...
\begin{itemize}
    ...
    \item \textbf{Gemini 3 Flash}: Commercial model from Google DeepMind
\end{itemize}
...
\begin{itemize}
    \item \textbf{Alternate Uses Task (AUT)}: Generate creative uses for common objects (4 items)
    \item \textbf{Scientific Creative Thinking Task (SCTT)}: Generate hypotheses and research questions for scientific scenarios (4 items)
    \item \textbf{Design Problem-Solving}: Propose solutions to real-world design challenges (4 items)
...
For story responses, we employed two complementary metrics: (1) \textbf{Divergent Semantic Integration} (DSI), measuring average pairwise semantic distance between words in the story, and (2) \textbf{MAoSS} (Multilingual Assessment of Short Stories), an LLM-based creativity rating. This dual approach allowed us to examine whether distributional patterns differ across scoring methodologies.
...
CRPO's advantage on ideation tasks (AUT, SCTT, Design) likely reflects task-specific training. The MuCE dataset contains substantial alternate uses and short-form ideation data, providing CRPO direct exposure to human response patterns on these formats. Base models, whose training data includes a significant amount of narrative text, show comparatively better alignment on story writing. Additionally, the MuCE dataset contains fewer story responses relative to ideation tasks, which may contribute to CRPO's reduced alignment with human patterns on this task type.
...
A striking dissociation emerged between originality and effectiveness metrics. All models exceeded human originality scores substantially, yet CRPO matched human effectiveness ratings almost perfectly ($d = 0.03$, $p = .55$). Notably, CRPO was trained to predict human originality ratings, not effectiveness. However, originality and effectiveness are positively correlated on the Design task, which may explain why CRPO's alignment with human originality preferences also produces human-like effectiveness distributions. This dissociation warrants careful consideration: if AI scorers systematically favor LLM outputs on originality, they may be unsuitable for evaluating simulacra validity without appropriate recalibration.
...
The tasks used in this study overlap with CRPO's training data: AUT, SCTT, Design, and one story prompt (pen-paper-write) appeared in the MuCE dataset. While the specific items varied (e.g., novel story prompts were used), this overlap limits our ability to fully distinguish learned human-like patterns from task-specific memorization. CRPO's consistent performance across held-out items suggests some generalization, but stronger evidence requires testing on entirely new task types absent from training.
...
\textbf{Training data overlap.} The task types used in this study (AUT, SCTT, Design, Story) appeared in CRPO's training data, though specific items varied. This overlap limits our ability to fully distinguish learned human-like patterns from task-specific memorization.
...
\begin{enumerate}
    \item \textbf{Novel task generalization}: Testing CRPO on creativity tasks entirely absent from its training data (e.g., metaphor generation, visual creativity descriptions) would provide stronger evidence for human-like creativity transfer beyond task-specific learning
