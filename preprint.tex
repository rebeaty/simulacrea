...
\title{Simulating Human Creativity: \\
Evaluating LLMs as Psychometric Simulacra}
...
\textbf{Human sample.} We recruited approximately 240 participants via Prolific, yielding 4,683 matched responses across four creativity tasks (one response per item per participant, randomly selected to match LLM sampling constraints).

\textbf{Model configurations.} We evaluated three LLM configurations:
\begin{itemize}
    \item \textbf{CRPO}: Llama 3.1 fine-tuned via Creative Preference Optimization on human creativity responses from the MuCE dataset \citep{ismayilzada2025crpo}
    \item \textbf{Llama 3.1 8B}: Base model without creativity-specific training
    \item \textbf{Gemini 3 Flash}: Base model from Google DeepMind
\end{itemize}
...
\begin{itemize}
    \item \textbf{Alternate Uses Task (AUT)}: Generate creative uses for common objects
    \item \textbf{Scientific Creative Thinking Task (SCTT)}: Generate hypotheses and research questions for scientific scenarios
    \item \textbf{Design Problem-Solving}: Propose solutions to real-world design challenges
    \item \textbf{Story Writing}: Write short creative stories incorporating specified words
\end{itemize}

\subsection{Scoring}

Responses were evaluated using the Creative Assessment Platform (CAP), an AI-based scoring system providing \textbf{originality} ratings (AUT, SCTT, Design) and \textbf{effectiveness} ratings (Design only).

For story responses, we employed two complementary metrics: (1) \textbf{Divergent Semantic Integration} (DSI), measuring average pairwise semantic distance between words, and (2) \textbf{MAoSS}, an AI-based creativity rating. This dual approach allowed us to examine whether distributional patterns differ across scoring methodologies.

\subsection{Analysis}

Our primary outcome measure was \textbf{distributional similarity} between model and human responses, quantified via:
\begin{itemize}
    \item Cohen's $d$ (pooled SD) for effect size estimation
    \item Independent samples $t$-tests for mean comparisons
    \item Semantic centroid similarity using sentence embeddings
\end{itemize}
...
Across metrics, CRPO emerged as the most human-like model on 4 of 6 measures (Table~\ref{tab:summary}), supporting our hypothesis that preference-optimized training produces more valid simulacra.

\begin{table}[!t]
\centering
\footnotesize
\caption{Most and least human-like model by metric}
\label{tab:summary}
...
Metric & Best & Worst \\
\midrule
AUT Orig. & CRPO & Gemini \\
SCTT Orig. & CRPO & Gemini \\
Design Orig. & CRPO & Gemini \\
Design Eff. & CRPO & Llama \\
Story (DSI) & Llama & Gemini \\
...
\textbf{Population representativeness.} Human participants were recruited via Prolific, a convenience sample that may not represent the full range of human creative ability. Similarly, the 245 synthetic ``participants'' per model represent a limited sampling of each model's output distribution.
